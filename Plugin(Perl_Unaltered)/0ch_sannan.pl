	...
	if ($type & (16 | 32)) {
		...
		if ($type&16) {
			...
			if ($ipAddr !~ /:/) {
				...
				my $res = whois($ipAddr);
				for my $line ($res) {
					$country = $1 if $line =~ /country:.*([A-Z]{2})/i;
				}
			}
...
sub BBS_SLIP
{
	...
	# bbs_slipの初期設定
	my $slip_id = '';
	my $slip_nickname = "ﾜ${fixed_nickname_end}";
	my $slip_aa = $slip_ip;
	...
	if (!$slip_remoho || $ipAddr eq $remoho) { # 逆引きできない場合
		...
		if (!$remoho_checked && $isAnon) {
			$idEnd = '8';
			$slip_nickname = $Sys->Get('BBS') =~ /^live/ ? 'ｺﾝﾆﾁﾊ' : 'anon';	
		...
		if (!$remoho_checked && $ismobile) {
				# モバイル回線のslip_id
				$slip_id = 'MM';
				$slip_id = 'Sr' if $ismobile =~ /^(?:om|ｵｯ|sb|ﾊｹﾞ)/;
				$slip_id = 'Sp' if $ismobile =~ /^(?:pw|ｻｻ)/;
				$slip_id = 'Sa' if $ismobile =~ /^(?:au|ｱｳｱｳ)/;
				$slip_id = 'Sd' if $ismobile =~ /^(?:sp|ｽ)/;
				$slip_id = 'SD' if $ismobile =~ /pera|ﾍﾟﾗ/;
				$idEnd = substr($slip_id, -1, 1);
				$slip_nickname = "${ismobile}${mobile_nickname_end}";
				$slip_aa = $slip_id;
				$slip_bb = $slip_ip;
...
sub Ninpocho
{
	...
	if (!$capID) {
		...
		# game GR2-30 スタレ
		#PrintBBSError_Ninja($Sys, 10007, $koyuu) if $ninlv < 5 && $ismobile eq 'ﾃﾃﾝﾃﾝﾃﾝ' && $ua =~ /2201116SR/ && $bbs eq 'game';

		# game 静岡
		#PrintBBSError_Ninja($Sys, 10007, $koyuu) if $ninlv < 5 && ($ismobile eq 'ﾃﾃﾝﾃﾝﾃﾝ' || $remoho =~ /tubecm00.ap.so-net.ne.jp/) && $ua =~ /SM-G973C/ && $bbs eq 'game';
			...
			if (open(my $fh, "<", $envban_txt)) {
				while(my $line = readline $fh){ 
					...
					if ($line =~ /([^:]+):([^:]+):([^:]+)/) {
						...
						$remoho_match = 1 if $target_remoho eq 'TETEN' && ($ismobile eq 'ﾃﾃﾝﾃﾝﾃﾝ' | $ismobile eq 'ten');
	...
	if ($msg ne "" ) {
		my @icoNames = (
			...
			'8_2.gif',
			'8k.gif',
			'8toushinnomonar16.gif',
			'8toushinnomonar32.gif',
			'a1.gif',
			'a2.gif',
			'ace.gif',
			'af1.gif',
			...
			'anime_giko13.gif',
			'anime_hossyu01.gif',
			'anime_imanouchi01.gif',
			'anime_imanouchi02.gif',
			'anime_imanouchi03.gif',
			'anime_imanouchi04.gif',
			'anime_iyahoo.gif',
			'anime_iyou01.gif',
			'anime_iyou02.gif',
			'anime_jien01.gif',
			...
			'anime_jien03.gif',
			'anime_jien04.gif',
			'anime_jyorujyu01.gif',
			'anime_jyorujyu02.gif',
			'anime_jyorujyu03.gif',
			'anime_karake.gif',
			'anime_kukkuru01.gif',
			'anime_kuma01.gif',
			'anime_kumaface01.gif',
			'anime_loop.gif',
			'anime_marara01.gif',
			'anime_marara02.gif',
			'anime_marara03.gif',
			...
			'anime_morara06.gif',
			'anime_nokar01.gif',
			'anime_nurupo_ga_1.gif',
			'anime_nurupo_ga_2.gif',
			'anime_okashi01.gif',
			'anime_okashi02.gif',
			'anime_onigiri01.gif',
			'anime_onigiri02.gif',
			...
			'anime_wacchoi03.gif',
			'anime_wacchoi04.gif',
			'anime_youkanman01.gif',
			'anime_youkanman02.gif',
			'anime_youkanman03.gif',
			'anime_zonu01.gif',
			'anime_zonu02.gif',
			'ao.gif',
			'apple.gif',
			...
			'hagenin-jump.gif',
			'hagenin-mount.gif',
			'hagenin-shuriken.gif',
			'hageshikuninja32.gif',
			'hagurumaou.gif',
			'hagurumaou_g16.gif',
			'hagurumaou_g32.gif',
			'hagurumaou_s32.gif',
			...
			'kasiwamoti1.gif',
			'kasiwamoti2.gif',
			'kinokorusensei32.gif',
			'kit_megane.gif',
			'kit_otsudearimasu.gif',
			'kita.gif',
			'kita1.gif',
			'kita2.gif',
			'kita_.gif',
			...
			'ninzya2.gif',
			'niraimu32.gif',
			'niramushigotokiga16.gif',
			'niramushigotokiga32.gif',
			'niratyaneko32.gif',
			'nono.gif',
			'nono_dash.gif',
			'nono_face.gif',
...
sub Nusi16
{
	...
	if ($Sys->Equal('MODE', 2)) {
		...
		if ($noriben && !$capID && !$isowner) {
			...
			$msg =~ s/ア.*ベ.*シ.*ン.*ゾ(?:.*ウ)?|あ.*べ.*し.*ん.*ぞ(?:.*う)?|シ.*ン.*ゾ.*ア.*ベ|ｱ.*ﾍﾞ.*ｼ.*ﾝ.*ｿﾞ(?:.*ｳ)?|ｼ.*ﾝ.*ｿﾞ.*ｱ.*ﾍﾞ|a.?b.?e.?s.?(?:h.?)?i.?n.?z.?o.?u?/【&#9608;&#9608;&#9608;&#9608;】/gi;
	...
	if ( $bbs eq 'liveuranus' && ($Threads->GetAttr($threadid, 'mirror') || $name =~ /!mirror/) && $session && !$Threads->GetAttr($threadid, 'nomirror') && !$Threads->GetAttr($threadid, 'noid') && $msg !~ /&gt;&gt;/ ) {
		...
		if ($ninlv > 4 && ! -f $mirror_log) {
			...
			EARENDIL::Copy($mirror_txt, $mirror_log);
		}
	}
	...
	if ($isowner && $name !~ /!nocmd/) {
		...
		if (!$isSub && !$capID) {
			...
			# exp1r
				...
				EARENDIL::Copy("$bbsPath/dat/$threadid.dat", $dstDat);
				unlink "$bbsPath/dat/$threadid.dat";
