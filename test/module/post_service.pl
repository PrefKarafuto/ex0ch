...
sub AddTimeLine {
    ...
    if(open(my $fh, '>', $TLpath .'/'. $crypt_filename . ".cgi")){
		flock($fh, 2);
		chmod 0600, "$TLpath/$crypt_filename.cgi";
		$lines[4] //= $title;
		$lines[5] //= $url;
		$line = join('<>',@lines);
		print $fh "$line";
...
sub OMIKUJI
{
	...
	if ($name =~ /!omikuji/) {
		my @list = (
			...
			[ '【樺太】',  '1/15' ],
			[ '【千島】',  '1/15' ],
			[ '<span style="color:#F9C">【男の娘】</span>',  '1/80' ],
			[ '<span style="color:#960">【腐女子】</span>',  '1/200' ],
			[ '<span style="color:#CCC">【髪】</span>',  '1/400' ],
			[ '<span style="color:#C90">【神】</span>',  '1/800' ],
			[ '<span style="color:#C69">【姫君】</span>',  '1/2000' ],
			[ '<span style="color:#F3C">【女神】</span>',  '1/3000' ],
			[ '<span style="color:#336">【尊師】</span>',  '1/40298' ],
			[ '<span style="color:#CCC">【勘察加】</span>',  '1/270000' ],
		);
		...
		# 重みをパースして数値化しつつ、合計を計算
		my @weights;
		my $total = 0;
		for my $pair (@list) {
			my $w = $pair->[1];
			if (defined $w && $w =~ m{^(\d+(?:\.\d*)?)/(\d+(?:\.\d*)?)$}) {
				$w = $1 / $2;      # 分数形式を小数に
			} else {
				$w = $w + 0;       # それ以外は数値キャスト
			}
		...
		# 重みを順に引いて、最初に r < weight となった要素を返す
		for my $i (0 .. $#list) {
			return $list[$i][0]
				if $r < $weights[$i];
			$r -= $weights[$i];
