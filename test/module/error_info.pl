...
sub Load {
	...
	my $messages = {
		...
		'153' => { SUBJECT => '認証されてない', MESSAGE => 'Captcha認証をしてください。<br>専用ブラウザからの場合は、一度通常ブラウザからCaptcha認証を行い、メール欄に<br>!auth<br>と入れて書込みをしてください。ワンタイムパスワードを発行します。' },
		'154' => { SUBJECT => '認証失敗', MESSAGE => 'Captcha認証に失敗しました。' },
		'155' => { SUBJECT => 'ユーザー認証失敗', MESSAGE => 'ユーザー認証に失敗しました。' },
		'158' => { SUBJECT => '認証されてない', MESSAGE => 'ユーザー認証をしてください。<br>専用ブラウザからの場合は、一度通常ブラウザからCaptcha認証を行い、メール欄に<br>!auth<br>と入れて書込みをしてください。ワンタイムパスワードを発行します。' },
		'159' => { SUBJECT => '認証用ワンタイムパスワード発行', MESSAGE => 'パスワードは<br><br>!auth:{!PASSWORD!}<br><br>です。この書き込みから3分以内にメール欄にパスワードを入力して、専用ブラウザから書き込んでください。' },
