...
sub Load {
	...
	my $messages = {
		...
		'158' => { SUBJECT => '認証されてない', MESSAGE => 'ユーザー認証をしてください。<br>通常ブラウザで、コマンド欄に<br><br>!auth<br><br>と入れて書き込みをしてください。認証用パスワードを発行します。' },
		'159' => { SUBJECT => '認証用ワンタイムパスワード発行', MESSAGE => 'パスワードは<br><br>!auth:{!PASSWORD!}<br><br>です。この書き込みから3分以内にコマンド欄にパスワードを入力して、専用ブラウザから書き込んでください。' },
		'160' => { SUBJECT => '忍法帖保存ページ', MESSAGE => '時刻:{!TIME!} (UTC)<br>あなたの忍法帖パスワード:{!NIN_PASS!}<br><br>これはあなたの忍法帖パスワードなので大切に保管してください。<br>最新のパスワードのみが有効です。キャンセルを押して戻ってください。' },
